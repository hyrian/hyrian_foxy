...
class ConversationStateMachine(Node):
    def __init__(self):
        ...
        openai.api_key_path = '/home/ubuntu/Documents/openapi_key.txt'
        self.request_text = ""
        self.arrival_flag = 0
        self.keyword = None
    ...
    def run_state_machine(self):
        try:
            if self.current_state == "WAITING":
                if self.previous_state != "WAITING":  # 이전 상태가 'WAITING'이 아니었다면 로그 출력
                    self.get_logger().info("상태가 WAITING입니다.")
                self.previous_state = "WAITING"  # 현재 상태를 이전 상태로 업데이트
                ...
                # self.keyword 사용 (이미 "PROVIDING_INFORMATION"에서 설정됨)cl
    ...
    def speak_long_response(self, filename):
        text_path = '/home/ubuntu/feature_ws/src/hyrian_foxy/hyrian_gpt/text'
