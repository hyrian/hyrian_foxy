...
class HyrianNode(Node):
  def __init__(self):
    ...
    _port_name = self.get_parameter_or('port.name', Parameter('port.name', Parameter.Type.STRING, '/dev/ttyS0 ')).get_parameter_value().string_value #일단 임시로 바꿈 아래 줄 디버깅하려고
  ...
  def cbCmdVelMsg(self, cmd_vel_msg):
    ...
    print("Received cmd_vel message:", cmd_vel_msg)
    ##############################################################################
