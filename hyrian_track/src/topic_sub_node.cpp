...
public:
  TopicSubscriber()
    : Node("topic_subscriber"),tracking_gain(0.3),error_x(0.0), cmd_vel_msg(geometry_msgs::msg::Twist())
  {
...
private:
  ...
  { 
    ...
    pass.setFilterLimits(0.45, 1.2);     // 원하는 거리 값의 범위
    pass.filter(*cloud);
    ...
    else
    {
      ...
      if (min_distance > 40) // SOME_LARGE_VALUE는 적절한 임계값으로 설정합니다.
      {
  ...
  void human_tracking()
  {
      ...
      if (this->distance <= 0.8 && this->distance >= 0.6) // 사람과의 거리가 0.7 이하면 로봇을 멈춥니다.
      { 
        RCLCPP_INFO(this->get_logger(), "too close");
        stop_robot();
      }
      else if(this->distrance < 0.6) // 후진
      {
