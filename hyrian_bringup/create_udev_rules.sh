...
if [ -f "/etc/udev/rules.d/96-hyrian-imu.rules" ]; then
    ...
    echo 'SUBSYSTEM=="ttyUSB*", ATTRS{idVendor}=="10c4", ATTRS{idProduct}=="ea60", ATTRS{serial}=="b4f8aa510b9eec119c81738a43a0c072", MODE:="0666", GROUP:="dialout", SYMLINK+="ttyIMU"' > /etc/udev/rules.d/96-hyrian-imu.rules
